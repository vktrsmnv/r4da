...
This page contains the mathematical details for deriving the least-squares estimates for slope ($\beta_1$) and intercept ($\beta_0$).
We obtain the estimates, $\hat{\beta}_1$ and $\hat{\beta}_0$ by finding their values that minimize the sum of squared residuals, as shown in @eq-ssr.

$$
SSR = \sum\limits_{i=1}^{n}{\underbrace{[\underbrace{y_i}_{\text{Observation } i} - \underbrace{\hat{y}_i}_{\text{Predicted } i}}_{\text{Residual}}}]^2 = [y_i - (\hat{\beta}_0 + \hat{\beta}_1 x_i)]^2 = [y_i - \hat{\beta}_0 - \hat{\beta}_1 x_i]^2
$$ {#eq-ssr}

We can find the values of $\hat{\beta}_1$ and $\hat{\beta}_0$ that minimize /eq-ssr by taking the partial derivatives of @eq-ssr and setting them to 0.
